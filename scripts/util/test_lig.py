...
input_pdb="./6UPJ_H.pdb"
PATH_DIRECTORY ="./TEST_out/"

in_dir = './out_all_chain_with_h/'
out_dir = './out_with_lig_def/' # PATH_DIRECTORY
cache_dir = './cache_lig_def/'
...
for fname in os.listdir(in_dir):
    ...
    if 1==1:
        ...
        for chain in chains:
            try:
                io.set_structure(structure)
                print(out_dir+fname.split('_')[0]+'_'+str(chain)+"_H.pdb")
                io.save(out_dir+fname.split('_')[0]+'_'+str(chain)+"_H.pdb",ChainSelect(chain))
                ligand=get_pdb_components(out_dir+fname.split('_')[0]+'_'+str(chain)+"_H.pdb")
                ligand = ligand.getResnames()
                ligand = list(set(ligand))[0]
                os.system('mv '+out_dir+fname.split('_')[0]+'_'+str(chain)+"_H.pdb"+' '+out_dir+fname.split('_')[0]+'_'+ligand+"_H.pdb")
            except:
                if os.path.isfile(out_dir+fname.split('_')[0]+'_'+str(chain)+"_H.pdb"):
                    os.system('rm '+out_dir+fname.split('_')[0]+'_'+str(chain)+"_H.pdb")
