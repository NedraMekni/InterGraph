...
def get_dicts():
    ...
    pytg_graph_dict = {}
    multiligand_pdbs = []
    with open("multiligand_pdb.txt", "r") as f:
        multiligand_pdbs = eval(f.readline())
    for f in files:
        local_G = torch.load(f)
        for k in local_G.keys():

            if k.split("_")[0] in multiligand_pdbs:
                continue

            feat_k = local_G[k][1]
            pytg_graph_dict[k] = local_G[k][0]
            new_x = [
                reduce(lambda x, y: x + y, feat_k[int(el.item())]["attributes"])
                for el in pytg_graph_dict[k].x
            ]

            pytg_graph_dict[k].x = torch.tensor(
                new_x, dtype=torch.float
            )  # ,device=device)
            del feat_k, new_x
        ...
        del local_G
    print("OK")
    print("Dataset length: {}".format(len(pytg_graph_dict.keys())))
...
if __name__ == "__main__":
    ...
    tail_fname = "_complete_4_1k_2adamw_max_pool_seed50_bl.txt"
    # global_G = torch.load(ROOT_PATH+'/cached_graph/tensorG_0.pt')
    pytg_graph_dict = get_dicts()
    print(
        "end loading\n Dataset dimension <nGraphs> = {}".format(
            len(pytg_graph_dict.keys())
        )
    )
    ...
    for k in pytg_graph_dict.keys():
        ...
        try:
            ...
            print(
                "{} removed for error in reading edges - probably empty edge list".format(
                    k
                )
            )
            continue
    ...
    for i in range(k):
        ...
        model.to(device)
        # optimizer = torch.optim.SGD(model.parameters(), lr= 0.0001, momentum=0.9) # Define optimizer.
        # optimizer = torch.optim.Adam(model.parameters(), lr=0.000001)
        # old learning rate 0.00001
        ...
        train_set = [
            graph
            for graph in graph_list
            if graph not in test_set and graph not in validation_set
        ]

        train_set = train_set[: (len(train_set) // batch_size) * batch_size]
        validation_set = validation_set[
            : (len(validation_set) // batch_size) * batch_size
        ]
        ...
        print("n validation set {}".format(len(validation_set)))
        print("n test set {}".format(len(test_set)))
        ...
        for epoch in range(EPOCH):
            ...
            with open(
                "train_ki_ref_out_train_relu_batch_32_lr{}".format(tail_fname), "a"
            ) as f:
                f.write("{},{}\n".format(ref_g_train, out_g_train))
            with open(
                "train_ki_ref_out_val_relu_batch_32_2_lr{}".format(tail_fname), "a"
            ) as f:
                f.write("{},{}\n".format(ref_g_val, out_g_val))
            torch.cuda.empty_cache()
        with open(
            "train_ki_external_relu_batch_32_2_lr{}".format(tail_fname), "w"
        ) as f:
            f.write(str(res_train))
        with open(
            "validation_ki_external_relu_batch_32_2_lr{}".format(tail_fname), "w"
        ) as f:
            f.write(str(res_val))
        ...
        with open("test_ki_external_relu_batch_32_2_lr{}".format(tail_fname), "w") as f:
